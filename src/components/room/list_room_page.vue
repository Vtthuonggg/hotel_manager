<template>
  <div class="container">
    <v-row class="row-title-item">
      ...
      <v-btn class="gradient-button" @click="showCreateRoom" style="color: white">
    ...
    <v-row justify="center" class="row-item">
      <v-col cols="12" md="6" lg="4">
        <v-card class="status-card">
          <v-list-item>
            <v-icon style="color: #23b84f" large>mdi-check-circle-outline</v-icon>
            <v-col>
    ...
    <v-container v-if="rooms.length > 0">
      <v-row>
        <v-col v-for="(room, index) in rooms" :key="index" cols="12" sm="6" md="4" lg="2">
          <v-menu
            v-model="menu"
    ...
    <div v-if="isShowCreateRoom" class="popup">
      <div class="popup-content">
        ...
        <v-text-field v-model="newRoom.price" label="Giá phòng" v-mask="dynamicMask"></v-text-field>
        ...
        <div class="button-container">
          <v-btn class="gradient-button-cancel" style="color: #007bff" @click="hideCreateRoom">Hủy</v-btn>
          <v-btn class="gradient-button-confirm" style="color: white" @click="addRoom">Xác nhận</v-btn>
        </div>
      </div>
    </div>
  </div>
